...
export class UI {
	...
	protected async updateCheck() {
		if (_UserScript_ && user.userStatus!.bilibiliplayer && user.userStatus!.checkUpdate) {
			const version = await GM.getValue('biliPlayerCommit');
			if (version !== _PlayerCommit_) {
	...
	protected initSettingCommon() {
		this.menuitem.common.addSetting([
			...
			this.switch('bilibiliplayer', '重构播放器', '修复及增强', svgPlay, v => {
				if (v) this.updateCheck();
			}, '旧版播放器已于 2019-10-31T07:38:36.004Z 失去官方维护，为了旧版播放器长期可持续维护，作者使用 TypeScript 完全重构了旧版播放器。修复了旧版播放器出现异常或失效的功能（如无法获取 90 分钟以后的弹幕问题），移植了一些B站后续推出的功能（如互动视频、全景视频、杜比视界、杜比全景声、AV1 编码支持和 DRM 支持等）。能力有限无法做到 100% 复刻，如果您想体验原生的旧版播放器，可以禁用本功能。同时由于项目托管于 Github，国内部分网络环境可能访问不畅，初次启动播放器可能耗时较久，加载失败后也会回滚原生播放器。如果您的网络环境始终无法正常加载，也请禁用本功能或者前往反馈。'),
			this.select('cdn', 'CDN', {
				candidate: ['Github', 'jsdelivr']
			}, '更新外部资源', svgNet, undefined, '用于加载外部资源的CDN，一般而言 jsdelivr 比 GitHub 好些，然而部分网路环境可能二者都无法访问。')
		]);
		...
		this.menuitem.common.addSetting([
			...
			}, 'access_key', undefined, undefined, '鉴权。效果等同于网页端的 cookie，B站服务器用以识别您的登录身份。本脚本只会在您本地保存鉴权，绝对不会泄露出去，唯一的例外是如果启用了【解除区域限制】功能并选择自定义服务器，则会向代理服务器发送鉴权，我们无法保证第三方服务器如何使用您的鉴权，所以万请自行必确认代理服务器的可信度，<strong>三思而后行</strong>！'),
	...
	protected initSettingRewrite() {
		this.menuitem.rewrite.addSetting([
			this.switch('av', 'av/BV', '恢复旧版 av 播放页'),
			this.switch('bangumi', 'bangumi', '恢复旧版 bangumi 页'),
			this.switch('watchlater', '稍后再看', '恢复旧版稍后再看'),
			this.switch('playlist', '播单', '恢复旧版播单页'),
			this.switch('index', '主页', '恢复旧版哔哩哔哩主页'),
	...
	protected initSettingDanmaku() {
		this.menuitem.danmaku.addSetting([
			this.switch('dmproto', '协缓弹幕', 'protobuf 弹幕支持', undefined, undefined, '因为B站已放弃维护 xml 弹幕，带来一些问题，比如 90 分钟后无弹幕，所以此项不建议禁用。【重构播放器】默认启用且不受此项影响。'),
			this.switch('dmwrap', '弹幕提权', '允许普权弹幕排版', undefined, undefined, '上古时代存在大量使用换行及空格等特殊字符来提权以达到高级弹幕效果的作品，在 html5 时代无法正常显示，启用此项将恢复普权弹幕排版效果。尽情享受弹幕艺术。【重构播放器】默认启用且不受此项影响。'),
			this.select('dmExtension', '弹幕格式', {
				candidate: ['xml', 'json']
			}, '拓展名', undefined, undefined, '【下载弹幕】及【本地弹幕】使用的弹幕格式，xml 是传统格式，json 是 protobuf 弹幕实际格式，前者一般拥有更小的体积，只是可能丢失彩蛋及部分非法字符。'),
			...
			}, '从其他视频加载弹幕', undefined, '从其他B站视频加载弹幕，可以输入关键 url 或者查询参数，如：<br/>av806828803<br/>av806828803?p=1<br/>aid=806828803&p=1<br/>ss3398<br/>ep84795<br/>注意：【重构播放器】此处加载的弹幕会替换【下载弹幕】的内容！'),
			this.button(<'dmContact'>'localVideo', '本地视频', () => {
				player.changeNaiveVideo();
			}, '播放本地磁盘上的视频', '打开', undefined, '播放本地磁盘上的 mp4 文件。视频及音频编码格式必须是你的设备及浏览器能够支持，推荐视频编码：AVC(H.264)；音频编码：AAC。配合【本地弹幕】或者【在线弹幕】功能等于是一个本地B站播放器~'),
			this.button(<'dmContact'>'localDm', '本地弹幕', () => {
				user.userStatus!.dmExtension === 'json' ? danmaku.localDmJson() : danmaku.localDmXml();
			}, '加载本地磁盘上的弹幕', '打开', undefined, '从本地磁盘上加载弹幕文件，来源可以是下载功能下载的弹幕，拓展名 .xml 或 .json，编码 utf-8。【合并弹幕】项能选择是否与播放器内已有弹幕合并。'),
			this.switch('danmakuProtect', '弹幕保护计划', '<a href="https://github.com/hmjz100/Bilibili-Old/blob/main/danmaku/README.md" target="_blank">查看目录</a>', undefined, undefined, '上古弹幕作品很多高级弹幕都丢失了，幸好本项目备份了一些。启用本功能将自动识别对应作品并使用【在线弹幕】功能加载备份的弹幕，找回曾经的感动。<br>※部分4:3视频请以播放器原始形态观看获取最佳体验，不推荐全屏。')
		])
	}
	/** 样式设置 */
	protected initSettingStyle() {
		this.menuitem.style.addSetting([
			...
			this.switch('fullBannerCover', '修正头图分辨率', '顶栏 banner 完整显示不裁剪', undefined, undefined, '旧版顶栏 banner 接口已不再更新，脚本使用新版 banner 接口进行修复，但二者图片分辨率不一致。脚本默认不会去动页面样式以尽可能原汁原味还原旧版页面，导致顶栏 banner 被裁剪显示不全，启用本项将调整顶栏分辨率以完整显示图片。'),
			this.switch('episodeData', '分集数据', 'bangumi', undefined, undefined, '显示bangumi分集播放和弹幕数，原始合计数据移动到鼠标浮动提示中。'),
			this.switch('simpleChinese', '繁 -> 简', '将繁体字幕翻译为简体', undefined, undefined, '识别并替换 CC 字幕繁体并转化为简体，使用内置的简繁对照表机械翻译。'),
	...
	protected initSettingRestore() {
		this.menuitem.restore.addSetting([
			this.switch('lostVideo', '失效视频', '尝试获取失效视频信息', undefined, undefined, '修复收藏的失效视频的封面和标题信息，并以红色删除线标记。使用缓存数据恢复的页面重构 av 页默认开启且不受此项影响，其中部分上古失效视频甚至还保留了评论区。'),
			this.switch('disableSleepChcek', '禁用直播间挂机检测', '就喜欢挂后台听个响不行吗！'),
			this.switch('show1080p', '不登录高画质支持', 'dash 模式限定', undefined, v => {
			...
			}, 'B站砍掉了不登录能获取的画质，最多只能获取 480P。您可以启用【账户授权】功能，授权本脚本使用您的登录信息，如此您退出登录后依然能获取高画质视频流。本功能只会在请求播放源时添加上登录鉴权，不影响页面其他功能的未登录状态，B站也不会记录您的播放记录。本功能适用于那些经常用浏览器无痕模式上B站的用户。若<strong>非常抱歉</strong>报错请关闭本选项！'),
			this.switch('timeLine', '港澳台新番时间表', '填充首页番剧板块', undefined, undefined, '在首页番剧板块中填充港澳台最新番剧更新信息。只提取了最新的 30 条番剧信息，所以数据中可能不会包含过于久远的更新。本功能只能显示已更新的番剧信息，而不能作为即将更新番剧的预测。'),
			this.switch('searchAllArea', '全区域搜索', '还原港澳台 Bangumi 结果', undefined, undefined, '替换搜索页面默认搜索结果，使能够搜索到港澳台 bangumi。需要特别注意搜索关键词，港澳台译名通常与简中译名不一致，但也不强制要求繁体，如：<br>别当欧尼酱了 -> 不当哥哥了/不當哥哥了<br>本功能只在默认搜索时生效，更改搜索范围无效。另外，启用本功能搜索到的 bangumi 会丢失 ep 列表。'),
			this.switch('watchlater2Av', '稍后再看重定向', '稍后再看重定向回 av 页'),
		]);
	}
	/** 播放设置 */
	protected initSettingPlayer() {
		this.menuitem.player.addSetting([
			...
			}, '播放速率调整拓展', undefined, '调节当前 HTML5 播放器速率，拥有比播放器自带的更宽的调整范围。注意：未调整前的当前速率默认为1。'),
			this.switch('webRTC', 'WebRTC', '<strong>关闭</strong>以禁用 p2p 共享带宽', undefined, undefined, 'B站使用【WebRTC】实现 p2p 共享，等同于将您的设备变成了B站的一个视频服务器节点，别人观看相同的视频或直播便可以从您的设备取流而不必访问B站固有的服务器。脚本默认<strong>关闭</strong>了此功能，以减轻小水管的带宽压力，如果您的带宽允许，还是推荐开启，人人为我，我为人人。bilibili~乾杯 - ( ゜-゜)つロ！'),
		...
		this.menuitem.player.addSetting([
			this.switch(<'videoLimit'>'videoLimit.status', '解除播放限制', '解除区域/APP限制', undefined, undefined, '内置服务器只能获取到 360P，有条件请在下面自定义服务器。'),
			...
			}, '大部分新视频【内置】服务器只能获取到 360P，实在不堪入目，有条件的话还是【自定义】服务器吧。对于大陆用户而言，【自定义】服务器一般填一个台湾就行，或者加上一个泰区。'),
		...
		this.menuitem.player.addSetting([
			this.select(<'uposReplace'>'uposReplace.th', '泰区', {
				candidate: upos
			}, '针对泰国（东南亚）限制视频', undefined, undefined, '泰区服务器 ban 了大陆 ip，所以必须选一个进行替换。卡加载时请酌情切换。'),
			this.select(<'uposReplace'>'uposReplace.gat', '港澳台', {
				candidate: ['不替换'].concat(upos)
			}, '针对港澳台限制视频', undefined, undefined, '港澳台视频服务器一般为大陆外的 Akamai，大陆用户有可能访问不畅，请按需酌情切换。若卡加载请关闭或者换一个。'),
			...
			this.select(<'uposReplace'>'uposReplace.download', '下载', {
				candidate: ['不替换'].concat(upos)
			}, '针对下载功能', undefined, undefined, '一般不需要替换，除非屡屡下载 403。若还是 403 请关闭或者换一个。'),
		], 3);
	}
	/** 下载设置 */
	protected initSettingDownload() {
		this.menuitem.download.addSetting([
			...
			this.chockboxs('downloadType', '请求的文件类型', ['mp4', 'dash', 'flv'], '视频封装格式', undefined, () => download.destory(), '勾选视频的封装类型，具体能不能获取到两说。封装类型≠编码类型：①mp4 封装，视频编码 avc+音频编码 aac，画质上限 1080P。②flv 封装，编码同 mp4，但可能切分成多个分段，须手动合并。③dash，未封装的视频轨和音频轨，以编码格式分类，aac 为音频轨（含 flac、杜比全景声），avc、hev 和 av1 为视频轨（任选其一即可），须下载音视频轨各一条后手动封装为一个视频文件。另外【解除区域限制】功能获取到的下载源不受本项限制。<br>※ 2022年11月2日以后的视频已经没有 flv 封装。'),
			this.switch('TVresource', '请求tv端视频源', '无水印', undefined, e => {
				e && alert('下载TV源必须将【referer】置空，否则会 403（无权访问）！另外浏览器不支持配置 UA 和 referer，请更换【下载方式】！', '403 警告', [
					{
						text: '置空 referer',
				...
				download.destory()
			}, '请求 TV 端下载源，唯一的优势是可能无 Bilibili 水印。注意：①B站 tv 端大会员不通用，所以可能无法获取到大会员视频或画质。②需要进行【账户授权】，否则只能以游客身份获取下载数据。③TV 源要求特定的 UA 且不能发送 referer，基本无法通过浏览器直接下载（403 无权访问），请选择其他下载工具。④mp4 封装的并非 tv 源。'),
			this.select('downloadQn', '画质', {
				candidate: ["0", "15", "16", "32", "48", "64", "74", "80", "100", "112", "116", "120", "125", "126", "127"]
			}, 'flv限定', undefined, () => download.destory(), '画质参数，只针对 flv 封装。mp4 封装没得选，dash 则由于特性会一次性提供所有画质选项。'),
			this.select('downloadMethod', '下载方式', {
				...
				switch (e) {
					case '浏览器':
						...
						break;
					case 'IDM':
						alert('<a href="https://www.internetdownloadmanager.com/" target="_blank">IDM（Internet Download Manager）</a>是 Windows 端著名的的下载工具，本方式将下载数据生成 IDM 导出文件，您可以在打开 IDM -> 任务 -> 导入 -> 从"IDM导出文件"导入开始下载。虽然有点麻烦，但是 IDM 支持配置 UA 和 referer，并且下载速度的确不是浏览器能比的。', 'IDM 导出文件');
						break;
					case 'ef2':
						alert('<a href="https://github.com/MotooriKashin/ef2" target="_blank">ef2</a>是原脚本作者 MotooriKashin 开发的一款开源的 IDM 辅助工具，支持直接从浏览器里拉起 IDM 进行下载，免去使用 IDM 导出文件的繁琐，同时解放你的鼠标左键。', 'ef2 辅助下载');
						break;
					case 'aria2':
						...
						break;
					case 'aria2rpc':
						alert('aria2 支持 rpc 方式进行下载，正确配置后方便程度不亚于 ef2 方式，唯一的问题是配置起来有亿点麻烦。</br>是否跳转到 rpc 相关设置？', 'aria2 rpc', [
							{
								text: 'RPC 配置',
			...
			}, '鉴权参数', undefined, undefined, '下载工具发送给服务器的身份标志，鉴权关键参数之一，无效的 User-Agent 将导致 403 无权访问。<strong>除非你知道自己在修改什么，否则请不要轻易调整此项。</strong>此项只在使用第三方下载方式时有效。'),
			this.input('referer', 'referer', {
				candidate: [location.origin]
			}, '鉴权参数', undefined, v => {
				v && alert('您勾选了下载 TV 源，根据经验必须将 referer 置空，不然会触发 403（无权访问）！是否撤销输入将 referer 置空？还是取消勾选 tv 源？', '设置冲突', [
					{
						text: '置空 referer',
						callback: () => user.userStatus!.referer = ''
					},
			...
			}, '下载时发送给服务器的标志之一，鉴权关键参数之一，无效的 User-Agent 将导致 403 无权访问。此项在网页端必须存在，而且一般为主站域名，但是 <strong>TV、APP 等源此项必须为空！</strong>'),
			this.input('filepath', '下载目录', {}, '保存下载文件的本地磁盘目录', undefined, undefined, 'ef2、aria2、aria2rpc 和 curl 方式限定。Windows 平台请注意使用反斜杠哦。')
		]);
		this.menuitem.download.addCard('aria2 相关');
		this.menuitem.download.addSetting([
			this.input(<'aria2'>'aria2.server', 'RPC 服务器', {
				prop: { type: "url", placeholder: "http://localhost" },
				candidate: ['http://localhost']
			}, '本地或远程链接', undefined, undefined, '端口号请另外输入。建议使用下方按钮测试 RPC 连接可用性。'),
			this.input(<'aria2'>'aria2.port', '端口', {
				prop: { type: "number", placeholder: "6800" },
				candidate: ['6800']
			}, '本地或远程端口', undefined, undefined, '服务器链接另外输入。建议使用下方按钮测试 RPC 连接可用性。'),
			this.input(<'aria2'>'aria2.token', 'token', {
				prop: { type: "password" }
			}, '鉴权', undefined, undefined, '如果 RPC 服务器启用了 token 鉴权的话。'),
			this.slider(<'aria2'>'aria2.split', '分段数目', {
				min: 1,
				max: 16,
				precision: 15
			}, '分段并发下载', undefined, undefined, '对于支持断点续传的文件，启用多线程同时并发下载通常是一种非常有效的提高下载速度的方法。如果需要请自行调整一个合适的并发数，1 表示不并发下载。值得注意的是，部分服务器会限制并发连接数目，并发连接过多有触发风控甚至被临时封禁的风险，所以并不是并发数越多越好。'),
			...
			this.button(<'aria2'>'aria2.test', '测试 RPC 连接', () => {
				...
				new Aria2().getVersion()
					...
					.catch(e => {
						msg.type = 'error';
						msg.push('> RPC 链接失败 ಥ_ಥ', e);
					})
					.finally(() => {
						msg.delay = 4;
					});
			}, '获取 aria2 信息', 'ping', undefined, '请确定正确配置并启用了 aria2 的 RPC 服务器。')
		], 1);
		this.menuitem.download.addCard('ef2 相关');
		this.menuitem.download.addSetting([
			this.switch(<'ef2'>'ef2.delay', '稍后下载', '添加到 IDM 下载队列但不开始', undefined, undefined, '要开始时请手动到 IDM 队列里点击开始。本项可以用来批量下载而不弹出多个窗口。注意：B站视频源使用的是临时链接，过期后无法访问，请及时下载或清理。'),
			this.switch(<'ef2'>'ef2.silence', '静默下载', '跳过 IDM 确认对话框', undefined, undefined, '默认情况下 IDM 会弹窗询问是否确认下载，在该确认框中可以调整保存目录和文件名等操作。启用本项以跳过该确认框。')
		], 2);
	}
	/**
	 * 新建开关设置
