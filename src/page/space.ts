...
export class PageSpace extends Page {
	...
	constructor() {
		...
		this.midInfo();
		this.updateDom();
		(<any>window).spaceReport = {};
		poll(() => document.querySelector('.new-space-entry'), d => d.remove());
	...
	/** 注册时间 */
	protected jointime() {
		poll(() => document.querySelector(".section.user-info") || document.querySelector(".info-personal"), t => {
			accountGetCardByMid(this.mid)
				.then(d => {
					const jointime = timeFormat(d.regtime * 1000, true);
					// 新版空间页面
					if (t.classList.contains('info-personal')) {
						new VdomTool(`<div data-v-31d5659a="" class="info-wrap">
							<span data-v-31d5659a="" class="info-command">注册</span>
							<span data-v-31d5659a="" class="info-value">${jointime}</span>
						</div>`).appendTo(t as HTMLElement);
					} else {
						// 旧版空间页面
